...
function RodentEnv(model::MuJoCo.Model)
    ...
    last_torso_x = torso.com[1]
    RodentEnv(MuJoCo.init_data(model), last_torso_x, sum(data.ctrl .^ 2))#, torso)
end

torso_x(model, env::RodentEnv) = MuJoCo.body(env.data, "torso").com[1]
torso_y(model, env::RodentEnv) = MuJoCo.body(env.data, "torso").com[2]
torso_z(model, env::RodentEnv) = MuJoCo.body(env.data, "torso").com[3]
torso_speed_x(model, env::RodentEnv) = (torso_x(model, env) - env.last_torso_x) / model.opt.timestep
...
function act!(model, env::RodentEnv, action, params)
    env.last_torso_x = torso_x(model, env)
...
function reward(model, env::RodentEnv, params)
    ...
    healthy_reward = params.healthy_reward_weight * (!is_terminated(model, env, params))
    ctrl_reward = -params.ctrl_reward_weight * env.ctrl_squared
    return forward_reward + healthy_reward + ctrl_reward
end

is_terminated(model, env::RodentEnv, params) = torso_z(model, env) < params.min_torso_z
...
info_fcns(::Type{RodentEnv}) = (;torso_x, torso_y, torso_z, torso_speed_x)
