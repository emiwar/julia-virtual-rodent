#!/bin/bash
#SBATCH --ntasks=1 --cpus-per-task 1 --mem-per-cpu 24000 -p gpu -t 0-06:00 -o %j_gpu.out -e %j_gpu.err --gres=gpu
...
srun --mpi=pmix -n 1 julia --project=. src/analysis/rollout_and_log_activations.jl j0zwbgns
