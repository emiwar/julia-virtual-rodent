...
params = parse_config(ARGS)#, "-wandb.run_name", "StandUp-{NOW}"])#ARGS)

#Setup the environment
walker = Environments.ModularRodent(;params.physics...)
template_env = Environments.FootPosEnv(walker)
...
template_state  = ComponentVector(Environments.state(template_env))
template_action = ComponentVector(Environments.null_action(template_env))
networks = Networks.FootPosNet(template_state, template_action; params.network...)
