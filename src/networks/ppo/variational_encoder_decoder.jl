struct VariationalEncDec{E,D,C} <: PPOActorCritic
    encoder::E
    decoder::D
    critic::C
end
...
function VariationalEncDec(template_env::Main.MuJoCoEnvs.MuJoCoEnv, params::NamedTuple)
    template_state = Main.ComponentTensor(Main.MuJoCoEnvs.state(template_env, params))
    action_size = length(Main.MuJoCoEnvs.null_action(template_env, params))
...
function actor(actor_critic::VariationalEncDec, state, params, action=nothing, latent_eps=nothing)
    ...
    imitation_target = Flux.ignore(()->state.imitation_target |> Main.array |> copy)
    proprioception = Flux.ignore(()->state.proprioception |> Main.array |> copy)
