...
function ParallelControl(template_state::ComponentVector, template_action::ComponentVector;
                         sigma_min=0.01, sigma_max=0.5, hidden_size=64)
...
function critic(pc::ParallelControl, states::ComponentArray)
    (
        hand_L = pc.critics.hand_L(states.hand_L |> autodiff_clean),
        hand_R = pc.critics.hand_R(states.hand_R |> autodiff_clean),
        arm_L = pc.critics.arm_L(states.arm_L |> autodiff_clean),
        arm_R = pc.critics.arm_R(states.arm_R |> autodiff_clean),
        foot_L = pc.critics.foot_L(states.foot_L |> autodiff_clean),
        foot_R = pc.critics.foot_R(states.foot_R |> autodiff_clean),
        leg_L = pc.critics.leg_L(states.leg_L |> autodiff_clean),
        leg_R = pc.critics.leg_R(states.leg_R |> autodiff_clean),
        torso = pc.critics.torso(states.torso |> autodiff_clean),
        head = pc.critics.head(states.head |> autodiff_clean),
    )
end
