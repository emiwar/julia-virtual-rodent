...
function state(env::ModularImitationEnv)
    current_target = interpolated_target(env, env.target_timepoint[]) |> NamedTuple
    future_target =  interpolated_target(env, env.target_timepoint[] + 0.1) |> NamedTuple
    
    #Transform root to egocentric coords
    rot_mat = torso_yawmat(env.walker) 
    current_target = merge(current_target, (;root=(pos = rot_mat * (current_target.root.pos - root_pos(env.walker)),
                                                   rot = subQuat(current_target.root.rot, root_xquat(env.walker)))))
    future_target = merge(future_target, (;root=(pos = rot_mat * (future_target.root.pos - root_pos(env.walker)),
                                                 rot = subQuat(future_target.root.rot, root_xquat(env.walker)))))
    prop = merge(proprioception(env.walker), (;root=(;dummy=0.0)))
    full_state = map(prop, current_target, future_target) do p, ct, ft
        (proprioception=p, current_target=ct, future_target=ft)       
    end
    return full_state
end
...
function compute_rewards(env::ModularImitationEnv)
    ...
    target = interpolated_target(env, env.target_timepoint[])
    root_dist = norm(target.root.pos - root_pos(env.walker))
    root_ang  = 2*acos(abs(clamp(target.root.rot' * root_xquat(env.walker), -1, 1)))

    prop = proprioception(env.walker)
    (
        hand_L = (
            finger_level = prop.hand_L.finger_zz,
            ...
            hand_pos = reward_shape(prop.hand_L.egocentric_hand_pos, target.hand_L.pos),
        ),
        arm_L = (
        ...
        hand_R = (
            finger_level = prop.hand_R.finger_zz,
            ...
            hand_pos = reward_shape(prop.hand_R.egocentric_hand_pos, target.hand_R.pos),
        ),
        arm_R = (
...
function status(env::ModularImitationEnv)
    prop = proprioception(env.walker)
    target = interpolated_target(env, env.target_timepoint[]) #.proprioception
    ...
    elseif target_frame(env)+1 >= clip_length(env)
        return TRUNCATED
    elseif norm(target.root.pos - root_pos(env.walker)) > env.max_target_distance
        return TERMINATED
    else
        return RUNNING
    end
...
function info(env::ModularImitationEnv)
    ...
    (
        ...
        hand_L_pos_error = norm(prop.hand_L.egocentric_hand_pos - target.hand_L.pos),
        foot_L_pos_error = norm(prop.foot_L.egocentric_foot_pos - target.foot_L.pos),
        ...
        torso_z_dot = dot(prop.torso.zaxis, target.torso.zaxis),
        leg_L_hip_height = prop.leg_L.hip_height,
        leg_R_hip_height = prop.leg_R.hip_height,
        pelvis_z_dot = dot(prop.torso.pelvis_zaxis, target.torso.pelvis_zaxis),
        root_ang = root_ang,
        root_dist = root_dist,
...
function interpolated_target(env::ModularImitationEnv, at_time=0.0)
    #at_time = (env.target_timepoint[] + time_ahead)
    precise_frame = at_time * env.target_fps
    int_frame = min(clip_length(env)-1, floor(Int64, precise_frame))
...
function precompute_target(walker::ModularRodent, orig_target::ComponentArray)
    ...
    template_prop = target_from_pose(walker) |> ComponentArray
    ...
    @Threads.threads for i = 1:n_clips
        ...
        for t = 1:dur
            ...
            new_target[:, t, i] = target_from_pose(w)
        end
    end
    return new_target
end

function target_from_pose(walker)
    target = proprioception(walker)
    (
        hand_L = (
            pos = SVector{3}(target.hand_L.egocentric_hand_pos),
            wrist_angle = target.hand_L.wrist_angle,
            finger_angle = target.hand_L.finger_angle,
            xaxis = SVector{3}(target.hand_L.xaxis),
        ),
        arm_L = (
            elbow_angle = target.arm_L.elbow_angle,
            elbow_height = target.arm_L.elbow_height,
            shoulder_height = target.arm_L.shoulder_height
        ),
        hand_R = (
            pos = SVector{3}(target.hand_R.egocentric_hand_pos),
            wrist_angle = target.hand_R.wrist_angle,
            finger_angle = target.hand_R.finger_angle,
            xaxis = SVector{3}(target.hand_R.xaxis),
        ),
        arm_R = (
            elbow_angle = target.arm_R.elbow_angle,
            elbow_height = target.arm_R.elbow_height,
            shoulder_height = target.arm_R.shoulder_height
        ),
        foot_L = (
            pos = SVector{3}(target.foot_L.egocentric_foot_pos),
        ...
        foot_R = (
            pos = SVector{3}(target.foot_R.egocentric_foot_pos),
        ...
        torso = (
            zaxis = SVector{3}(target.torso.zaxis),
            pelvis_zaxis = SVector{3}(target.torso.pelvis_zaxis),
        ...
        head = (
            xaxis = SVector{3}(target.head.xaxis),
            zaxis = SVector{3}(target.head.zaxis),
            egocentric_pos = SVector{3}(target.head.egocentric_pos),
            mandible = target.head.mandible
        ),
        root = (
            pos = root_pos(walker),
            rot = root_xquat(walker),
        )
    )
end
